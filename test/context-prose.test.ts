...
describe('a hedge with the path written between its two words', () => {
   ...
   *
   * The markers describe **prose**, and inline code is the claim rather than
   * the prose, so they are matched against the line with its code spans taken
   * out. That is one rule for the whole list instead of a second spelling of
   * each entry.
   */
  ...
  it('joins a marker a line wrap had split, which is the same gap sideways', () => {
    // The window is two lines, and collapsing the whitespace is what lets a
    // marker be read across the break between them.
    expect(
...
describe('a section that says its paths are somewhere else', () => {
   ...
   *
   * Priced before it was kept: over the 66 certification repositories and 700
   * others audited before and after, it removes no finding the line-level rules
