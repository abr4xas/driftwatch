...
(function () {
  ...
  if (cells && typeof CORPUS !== 'undefined') {
    /* The unit is the context file, not the repository. A grid of 66 equal
       ...
       The counts are computed from CORPUS rather than written here. They were
       written here once, and the page went on saying 236 files and 19 lit
       ones for two rounds after the corpus had moved to 341 and 29. The data
       file is generated by `pnpm site-data`; this reads it. */
    var buttons = [];
    var active = 0;
    var marked = 0;
    ...
    var totalFiles = 0;
    var litFiles = 0;
    for (var ri = 0; ri < CORPUS.length; ri++) {
      totalFiles += CORPUS[ri].s;
      var seen = {};
      for (var fi = 0; fi < CORPUS[ri].f.length; fi++) seen[CORPUS[ri].f[fi].f] = 1;
    ...
    /* The survey runs once, when it comes into view: the marks land in
       ...
       browser without one — leaves every mark at opacity zero and the section
